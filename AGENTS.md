...
## 4. UI Layer
*   **No Ad-hoc Styles:** Do not use random hex codes in UI components. Web code MUST use Tailwind Guna classes backed by `src/index.css` (`bg-avyakta-2`, `text-sattva`, `border-tamas-deep`, ...) and system accents via `getSystemAccent()` in `src/utils/theme.ts` (canonical hex registry: `src/theme/tokens.ts`, kept in sync with `index.css`). The legacy `useTheme()` hook does not exist — do not reference it in new code. Never build accent tints with a `${colour}15` template (invalid on CSS vars); use `accentTint()` from `src/components/Primitives.tsx`.
*   **Component Reuse:** Web shares `src/components/Primitives.tsx` — USE it, do not duplicate it. Disclosure chevrons (`DisclosureChevron`), row-navigation chevrons (`RowChevron`), breadcrumb chevrons (`BreadcrumbChevron`), accent count badges (`CountBadge`), chips (`ChipLink`), notices (`Notice`), collapsible sections (`CollapsibleSection`), bottom nav bars (`BottomBar`), action buttons/links (`ActionButton`/`ActionLink`), eyebrows (`Eyebrow`), cards (`Card`/`CardBody`) and shells (`PageShell`) all live there. New screens MUST compose these; ad-hoc `ChevronDown`/`ChevronRight` disclosure icons and local `Collapsible`/`RefSection` clones are forbidden. Every `transition-*` class MUST carry `motion-reduce:transition-none`; decorative icons MUST carry `aria-hidden="true"`; screen entrances MUST use `.animate-fade-in` (the `animate-in` plugin is not installed, so `animate-in fade-in` is dead). Keep layout/typography consistent with existing `src/components/*.tsx` patterns (max-w-3xl/4xl shells, Guna borders).

## 5. Language Convention — Indian English
*   **STRICT:** All user-facing prose, documentation, comments, and new content MUST use Indian English (British spellings): `colour`, `behaviour`, `centre`, `organise/organisation`, `recognise`, `realise`, `customise`, `utilise`, `favour/favourite`, `honour`, `labour`, `traveller/travelling`, `modelled/modelling`, `dialogue`, `catalogue`, `programme` (for a course of study; `program` only for computer code), `minimise/maximise/optimise/standardise`, `grey`, `cheque` (bank) vs `check` (verify).
*   **Code identifiers stay American:** Do NOT rename APIs, CSS variables (`--color-*`), Tailwind classes (`text-center`, `transition-colors`), or JS properties (`behavior`, `color`). The rule applies to prose only.
*   **Enforcement:** Prefer `-ise`, `-our`, `-re`, `-ll-`, `-ogue` forms in all new strings. Correct American forms when touching existing files.
