...
// Find the whole block from "// Compute bi-directional lookups" to the end
indexTs = indexTs.replace(/\/\/ Compute bi-directional lookups dynamically[^]*\}\);\n\}\);/, newLoop);

fs.writeFileSync('src/content/index.ts', indexTs);
console.log('Fixed TS error in index.ts');
